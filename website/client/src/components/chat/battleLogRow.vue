<template>
  ...
  >
    ...
    >
      <div class="d-flex">
        <!-- mf: TODO: somehow save the avatar in the db as rendered, it must be way faster than drawing it every time on base of all the gears etc -->
        <avatar
          v-if="user._id == msg.actorId && msg.actorType == 'user'"
          class="avatar-left log-avatar"
          :height="null"
...
import BattleLogMessageCard from '@/components/chat/battleLogMessageCard.vue';

// TODO merge chatMessages.vue (party message list) with messageList.vue (private message list)

export default {
  components: {
    BattleLogMessageCard,
    Avatar,
  },
